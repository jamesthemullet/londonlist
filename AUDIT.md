...
- [x] No e2e spec covers register → login → reset-password happy path — add `frontend/e2e/auth-flow.spec.ts` (existing specs `auth.spec.ts`/`public-pages.spec.ts` only test logged-out redirects and form validation, not a real submit) (found: 2026-09-01) (fixed: 2026-09-25 — `frontend/e2e/auth-flow.spec.ts` already existed on `main` with happy-path coverage for register, login, and forgot-password, but this checklist item was never checked off; verified all 7 cases pass and left the AUDIT.md line in sync with reality)
