...
- [ ] `frontend/jest.config.js`'s `collectCoverageFrom` excludes `pages/**` entirely, so `pages/museum/[id].tsx`, `my-list.tsx`, `login.tsx`, `register.tsx`, `reset-password.tsx`, `account.tsx`, `list/[username]/[listId].tsx`, `explore.tsx` etc. have unmeasured unit coverage — add `pages/**/*.{ts,tsx}` (excluding `_app.tsx`/`_document.tsx`/`api/**`) to surface real numbers (found: 2026-09-01)
