...
- [x] No e2e spec covers register → login → reset-password happy path — add `frontend/e2e/auth-flow.spec.ts` (existing specs `auth.spec.ts`/`public-pages.spec.ts` only test logged-out redirects and form validation, not a real submit) (found: 2026-09-01) (fixed: 2026-09-21 — `frontend/e2e/auth-flow.spec.ts` already covers register/login/forgot-password happy paths plus error states; verified all 7 specs pass via `yarn test:e2e`)
